...
const SACRED_MANTRA = 'OMBENZASATOHUNG';
...
const SecretAdminAccess: React.FC<SecretAdminAccessProps> = ({ isVisible, onClose }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const isMantraLogin = mantra.trim() === SACRED_MANTRA;

      if (isMantraLogin) {
        const result = await login({ email: 'mantra@bodhisattva.path' });
        if (result.success) {
        ...
        if (result.success) {
          ...
          setMessage(result.error ?? '❌ Invalid credentials. Only true Bodhisattvas may enter.');
        }
      } else {
        setMessage('❌ Enter credentials or sacred mantra.');
      }
    } catch {
