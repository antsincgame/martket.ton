...
**Сделано в сессии 2026-06-08 (PR #91–#119):** кода-блокеры mainnet, P2-харднинг
(Цепь Маркова), тёмная материя тестов, AML-мокап, in-container ops + verified через
Appwrite MCP. **Фаза 0.3** обогащена (#115). **Копилот-Lite** — `deriveNextAction`
на обоих лицах: машина (`/agent/status`+`/instructions`) и человек
(`/sellers/:wallet/onboarding` + `OnboardingGuide`), #116/#117 → **паритет
онбординг-наведения достигнут.** Тесты ~536 → 632.

### A. Покрытие MCP-поверхности (машина имеет Agent API, но не MCP-tool)
| # | tool | за ним | прио |
|---|---|---|---|
| A1 | `get_instructions` | `GET /agent/instructions` (0.1) | MVP |
| A2 | `get_status` | `/status` + `nextAction` (0.3 / C) | MVP |
| A3 | `create_product` | `POST /agent/products` (0.2) | MVP |
| A4 | `assistant_help` | мокап `POST /agent/help` | MVP |

### B. Самосуверенность машины (человек может, машина — НЕТ: нет Agent API)
| # | способность | человек | машина | вердикт |
|---|---|---|---|---|
| B1 | **Само-регистрация продавца** | `POST /sellers/register` | ❌ | **долг** — машина не онбордится сама до KYC-гейта |
| B2 | **BYOS-хранилище (профиль)** | `POST /storage` (R2/S3 + HeadBucket) | ❌ только per-listing | **долг** — машина не подключает хранилище сама |
| B3 | KYC-сессия | `POST /sellers/kyc/session` | ❌ | **намеренно** — владелец-человек обязателен (граница честности) |
| B4 | Asset-upload на платформу | `POST /listings/:id/asset` | ❌ | **намеренно** — TonForge BYOS, не хостинг |
| B5 | Buyer-поток (заказ/оплата) | `POST /orders`, `/confirm` | ❌ | **намеренно** — автономная покупка исключена (эскроу подписывает покупатель) |

### C. Паритет человека (машина имеет, человек в UI — нет)
| # | способность | вердикт |
|---|---|---|
| C1 | In-UI операционный мануал (секции инструкций) человеку | мелкий долг — карта-гид даёт *следующий шаг*, не весь мануал |
| — | `nextAction` структурно | ✅ паритет (#116/#117) |

### D. Ассистент (LLM-копилот)
| # | этап | вердикт |
|---|---|---|
| D1 | `assistant_help` мокап (оба лица) | долг (MVP) |
| D2 | LM Studio (локальная LLM, OpenAI-совместимо) | пост-MVP — нужен `LLM_BASE_URL` + сеть с Терры |

### Порядок исполнения (рекомендация)
1. **A1–A4 + D1** — 4 MCP-tool + мокап `/agent/help`. Низкий риск, MCP-first, замыкает
   покрытие продукта. ← ковать первым.
2. **B1 + B2** — агентские `register` + `storage`: глубже, **архитектурно/безопасностно
   значимо** (пускать ли машину само-регистрироваться до KYC-гейта владельца?). Развилка.
3. **C1** — мануал человеку (мелочь).
4. **D2** — LM Studio (пост-MVP, по сети с Терры).
